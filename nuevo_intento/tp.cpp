...
void split(string &cadena, string &sep, vector<string> &cadenaSplit){
    string copiaCad (cadena);
    string parteCadena;
    size_t pos;
    while ((pos = copiaCad.find(sep)) != string::npos) {
        parteCadena = copiaCad.substr(0, pos);
        cadenaSplit.push_back(parteCadena);
...
void PrioriColaInterpProtegida::encolar(InterpPriori *interpP){
    unique_lock<mutex> lock(this->centinela);
...
void HiloBF::run(){
    while (!this->heapInterp.estaVacia()){
        ...
        if (this->heapInterp.desencolar(iPriori)){
            iPriori.ejecutar();
            //delete iPriori; //deberia liberarse solo por RAII
        } else {
...
InterpPriori *ThreadPool::procesar_linea(string &linea){
    ...
    string rutaSalida = lineaSplit[3];
...
*/
Interpretador::Interpretador(string &rutaScript) : rutaScript(rutaScript){}
